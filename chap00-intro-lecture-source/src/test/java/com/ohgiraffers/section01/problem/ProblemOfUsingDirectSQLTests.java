...
class ProblemOfUsingDirectSQLTests {
    ...
    * 또한 객체를 사용할 때 SQL에 의존하면 객체에 값이 무엇이 들어있는지 확인하기 위해 SQL을 매법 살펴야 한다.
    * */
    ...
    * 3-1. 상속 문제
    * 객체 지향 언어의 상속 개념과 유사한 것이 데이터 베이스의 서브타입엔티티이다.
    ...
    * 하지만 객체지향의 상속은 슈퍼 타입의 속성을 공유해서 사용하므로 여기서 패러다임 불일치 현상이 발생한다.
    *
    * 법인 회원과 일반 회원이라는 두 가지 회원을 추상화하여 회원이라는 슈퍼타입 엔티티를 도출하고
    * 서브 타입 엔티티로 법인회원과 일반회원이 존재하는 상황인 경우
    * 물리적으로 회원과 법인 회원테이블로 분리된 상황에서는 각각 insert 구문을 수행해야 한다.
    * insert into 회원..
    * insert into 법인회원
    * 하지만 jpa를 이용하여 상속을 구현하는 경우에은
    ...
    * public class Menu {
    ...
    void testEquals() throws SQLException {
        ...
        Assertions.assertEquals(menu, menu2); // 주소값이 다르기 때문에 값이 같아도 false가 나온다.
    }
}