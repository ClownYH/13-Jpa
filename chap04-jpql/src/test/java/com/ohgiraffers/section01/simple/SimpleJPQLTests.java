...
public class SimpleJPQLTests {
    ...
    * jqpl은 엔티티 객체를 중심으로 개발할 수 있는 객체 지향 쿼리이다.
    * sql보다 간결하며 특정 dbms에 의존하지 않는다.
    * 방언을 통해 해당 DBMS에 맞는 SQL을 실행하게 된다.
    * jqpl은 find*() 메소드를 통한 조회와 다르게 항상 데이터베이스에 SQL을 실행해서 결과를 조회한다.
    * */
    ...
    * 키워드는 대소문자를 구분하지 않지만, 엔터티와 속성은 대소문자를 구분함을 유의한다.
    * 앤터티 별칭을 필수로 사용해야 하며 별칭 없이 작성하면 에러가 난다.
    * */
    ...
    *  - TypedQuery : 반환할 타입을 명확하게 지정하는 방식일 때 사용하며 쿼리 객체의 메소드 실행 결과로 지정한 타입이 반환된다.
    *  - Query : 반환할 타입을 명확하게 지정할 수 없을 때 사용하며 쿼리 객체 메소드의 실행 결과로 Object 또는 Object[]가 반환된다.
    ...
    * */
    ...
    void In_연산자를_활용한_조회_테스트(){
        ...
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).getResultList(); // 축약한다고 무조건 좋은 건 아니다. 가독성 우선
    ...
    void like_연산자를_활용한_조회_테스트(){
        ...
        // when
        String jpql = "SELECT m FROM menu_section01 m where m.menuName LIKE '%마늘%'"; // 인텔리제이는 고급 툴이라 색으로 처리해주나 보통 대소문자 구분으로 유지보수성을 올린다.
