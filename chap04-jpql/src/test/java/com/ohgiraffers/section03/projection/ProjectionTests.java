...
public class ProjectionTests {
    ...
    *   new 명령어를 사용한 클래스의 객체는 앤티티가 아니므로 영속성 컨텍스트에서 관리하지 않는다.
    * */

    // 1. 앤티티 프로젝션
    @Test
    void 단일_앤티티_프로젝션_테스트(){
        // when
        String jpql = "SELECT m FROM menu_section03 m";
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).getResultList();
        // 조회를 MyBatis로 할 경우 휠씬 수월해 혼용해서 쓰나 이 경우 그냥 쓰면 영속성 컨텍스트에서 관리하지 않는다.
        // 그래서 커밋에 변화를 주지 않게 되기 때문에 이를 방지하기 위해 PERSIST를 사용하는 방법도 있다.
        Assertions.assertNotNull(menuList);

        EntityTransaction entityTransaction = entityManager.getTransaction();
        entityTransaction.begin();
        menuList.get(1).setMenuName("test"); // 영속성 컨텍스트에서 조회한 결과를 가지고 있기 때문에 이렇게 동작한다.
...
//        Assertions.assertNotNull(categoryOfMenu.getMenuList());
    ...
    void 임베디드_타입_프로젝션_테스트(){
        ...
        // 임베디드 타입은 재사용성과 확장성 등을 고려하기 때문에 사용한다.
    }

    /* 3. 스칼라 타입 프로젝션 */
    @Test
    void TypedQuery를_이용한_스칼라_타입_프로젝션_테스트(){
     ...
     * 그때 TypedQuery 대신 Query를 사용하여 Object[]로 행의 정보를 반환 받아 사용한다.
     * */
    @Test
