...
* 2. insertable : 앤티티 저장 시 필드 저장 여부(default : true)
* 3. updatable : 앤티티 수정 시 필드 수정 여부(default : true)
* 4. table : 앤티티와 매핑될 테이블 이름. 하나의 앤티티를 두 개 이상의 테이블에 매핑할 때 사용(@SecondaryTable 사용)
* 5. nullable : null 값 허용 여부 설정, not null 제약조건에 해당함(true 기본값)
* 6. unique : 컬럼에 유일성 제약 조건
* (두 개 이상 컬럼에 unique 제약 조건을 설정하기 위해서 클래스 레벨에서 @Table의 uniqueConstraints 속성에 설정)
* 7. columnDefinition : 직접 컬럼의 ddl을 지정
* 8. lengh : 문자열 길이, String type에서만 사용(default : 255)
* */
...
public class Member {
    ...
    @Transient // 테이블 생성 시 무시된다. 자바에서는 필요한데 테이블에는 필요없는 데이터 때문에 존재하는 상태
    private String nickname;

    @Column(name = "phone", columnDefinition = "varchar(200) default '010-0000-0000'")
    private String phone;

    @Column(name = "email", unique = true) // 관계 설정은 나중에 alter 테이블로 들어간다. 어떤 에러가 발생할지 모르기 때문에
