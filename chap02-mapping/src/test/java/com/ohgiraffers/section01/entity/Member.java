...
* 또한, "name" 속성을 사용하여 앤티티 클래스와 매핑할 테이블의 이름을 지정할 수 있다.
* 생략하면 자동으로 클래스 이름을 앤티티명으로 사용한다.
*
* - 프로퍼티 내에 다른 패키지에도 동일한 엔티티가 존재하는 경우 해당 앤티티를 식별하기 위한 중복되지 않는 name을 지정해주어야 한다.
* - 기본 생성자는 필수로 작성해야 한다.
* - final 클래스, enum, interface, inner class에서는 사용할 수 없다.
* - 저장할 필드로 final을 사용하면 안된다.
* */
