...
public class ManyToOneAssociationTests {
    ...
    *
    * 방향에 따른 분류
    * 테이블의 연관 관계는 외래 키를 이용하여 양뱡향 연관 관계의 특징을 가진다.
    ...
    * */
    ...
    /*
    * ManyToOne은 다수의 엔티티가 하나의 앤티티를 참조하는 상황에서 사용된다.
    ...
    * 객체 지향 모델에 맞게 작성된 쿼리를 통해, 앤티티 객체를 대상으로 검색, 검색 결과를 토대로 객체를 조작할 수 있따.
    ...
    * 영속성 전이랑 특정 앤티티를 영속화할 때 연관된 엔티티도 함께 영속화한다는 의미이다.
    * cascade=Cascade.PERSIST를 설정하면 MenuAndCategory 앤티티를 영속화 할 때 Category 앤티티도 함께 영속화하게 된다.
    * */
    ...
    void 영속성_삭제_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class,100007);
        ...
        transaction.commit();
        MenuAndCategory deleteMenu = entityManager.find(MenuAndCategory.class,100007);
        Category foundCategory = entityManager.find(Category.class, 33338);
    ...
    void Refresh_테스트(){
         ...
         * */
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class,20); // 다른 cascade 옵션은 메뉴만 최신화시키고 카테고리는 기존 것을 쓴다. refresh는 이것도 해결한다.
    ...
    void Merge_update_테스트(){
        ...
        Category category = entityManager.find(Category.class,33340);
