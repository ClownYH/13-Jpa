...
public class BiDirectionTests {
    ...
    * 비즈니스 로직 상 더 중요하다고 연관 관계의 주인이 되는 것은 아니다.
    * 비즈니스 중요도를 배제하고 단순히 외래키 관리자의 의미를 부여해야 한다.
    * 연관 관계의 주인은 외래키를 가지고 있는 엔티티이다.
    *
    * 몌뉴 엔터티는 카테고리 엔터티의 카테고리 코드를 참조하고 있다.
    * 즉 외래키를 가지고 있는 메뉴 엔터티가 연관 관계의 주인이다.
    * */
    ...
    void 양방향_연관관계_매핑_조회_테스트(){
        ...
        // 가짜 연관 관계는 해당 엔티티를 조회하고 필요 시 연관 관계 앤티티를 조회하는 쿼리를 다시 실행하게 된다.
        ...
        // category에 포함된 메뉴 목록 출력 구문을 작성하고 나면 실제 사용에 필요해지기 때문에 가짜 연관 관계에 해당하는
        // 앤티티도 다시 조회하는 쿼리가 한번 더 동작한다.
        foundCategory.getMenuList().forEach(System.out::println);
    }

    /*
    * 연관 관계의 주인에는 값을 입력하고, 주인이 아닌 곳에는 값을 입력하지 않았을 때
    * 외래키 컬럼이 not null 제약 조건이 설정되어 있다면 에러가 발생한다.
    * 양방향 연관관계를 설정하고 흔히 하는 실숨이므로 반드시 menu 엔티티에 카테고리 정보를 추가한다.
    * */
    @Test
    void 양방향_연관관계_주인_객체를_이용한_삽입_테스트(){
        ...
        menu.setCategoryCode(entityManager.find(Category.class, 4)); // 연관관계의 주인이 아닌 것에 대한 값을 입력하지 않게 된다면 제약 조건에 위배되서 에러
...
//        category.getMenuList().add(insertMenu); // 주인이 아니기 때문에 주인에 입력한 값(insertMenu)을 입력할 수 없다.
